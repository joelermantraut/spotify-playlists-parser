...
class ArgParsing():
	...
	def run_scan(self):
		...
		songs = self.spoti_parser.search_artists_in_folders()
		if len(songs) > 0:
			self.presentation("Songs not in there Artist folder")
			self.save_on_file("Songs not in there Artist folder", self.spoti_parser, songs)
